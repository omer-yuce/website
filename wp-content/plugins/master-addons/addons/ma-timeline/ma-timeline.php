...
Customization Options.</span>'
		...
		protected function render() {
			...
			if ( ma_el_fs()->can_use_premium_code() ) {
				...
				$settings[] = $settings['ma_el_timeline_posts_per_page'];

				$offset = $settings['ma_el_timeline_post_offset'];

				$post_per_page = $settings['ma_el_timeline_posts_per_page'];
                            ...
                            <div class="ma-el-blog-timeline-slider">
                                <div class="ma-el-timeline-slider-inner">
                                    ...
                                    <?php
            		                    ...
            		                    if ( ( 'yes' === $item['ma_el_custom_timeline_custom_style'] && '' !== $item['ma_el_timeline_points_content'] ) ) {
...
//                                        }
		...
		protected function ma_timeline_points_image( $item = false ){
			...
			if( $item['ma_el_custom_timeline_pointer_image'] !="" ){
		...
		protected function ma_timeline_points_icon( $item = false ){
			...
			if ( $item ) {
				$has_item_icon = ! empty( $item['icon'] ) || ! empty( $item['ma_el_custom_timeline_selected_icon']['value'] );
			...
			if ( $item && '' !== $item['icon'] && $has_item_icon ) {
...
//			$excerpt_src  = $settings['ma_el_post_grid_excerpt_content'];
			...
			echo Master_Addons_Helper::ma_el_get_excerpt_by_id( get_the_ID(), $settings['ma_el_timeline_excerpt_length'], $excerpt_type, $this->parse_text_editor( $excerpt_text ), true );

		}

		/*
		 * Render Date
		 * Date output for post
		 * @since 1.1.5
		 */
